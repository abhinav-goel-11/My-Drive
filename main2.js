(function () {
    ...
    aRootPath.addEventListener("click",navigatBreadCrumb);
    ...
    function editFolder() {
      ...
      function deleteFolder() {
        let divFolder = this.parentNode;
        let divName = divFolder.querySelector("[purpose='name']");
        let ftbd = parseInt(divFolder.getAttribute("fid"));

        let flag = confirm("Are you sure you want to delete " + divName.innerHTML + "?");
        if (flag == true) {
            let exists = folders.some(f => f.pid == ftbd);

            if(exists == false){

                // ram
                let fidx = folders.findIndex(f => f.pid == ftbd);
    ...
    function viewFolder(){
       ...
       aPath.addEventListener("click",navigatBreadCrumb);
